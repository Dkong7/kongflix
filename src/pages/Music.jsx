...
export default function Music() {
  /* â”€â”€ NAVEGACIÃ“N SINCRONIZADA CON URL â”€â”€ */
  ...
  useEffect(() => { autoNextRef.current = handleAutoNext; });

  // Media Session — controles en notificación Android
  useEffect(() => {
    if (!current || !('mediaSession' in navigator)) return;
    const art = (current.artist && current.artist !== 'Desconocido') ? current.artist : (current.album || '');
    navigator.mediaSession.metadata = new MediaMetadata({
      title: current.title || 'KONGFLIX',
      artist: art,
      album: current.album || 'KONGFLIX',
      artwork: current.coverId ? [
        { src: 'https://drive.google.com/thumbnail?id=' + current.coverId + '&sz=w96', sizes: '96x96', type: 'image/jpeg' },
        { src: 'https://drive.google.com/thumbnail?id=' + current.coverId + '&sz=w256', sizes: '256x256', type: 'image/jpeg' },
        { src: 'https://drive.google.com/thumbnail?id=' + current.coverId + '&sz=w512', sizes: '512x512', type: 'image/jpeg' },
      ] : [],
    });
    ...
    navigator.mediaSession.setActionHandler('seekto', (d) => { if (d.seekTime != null) audioRef.current.currentTime = d.seekTime; });
  }, [current]);
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <div style={{ background: C.panel, borderBottom: `2px solid ${C.sand}`,
          ...
          <div style={{ position: 'relative', maxWidth: 480 }}>
            ...
            <input type="text"
                   placeholder={view === 'tracks' ? 'FILTRAR PISTAS...' : 'BUSCAR EN TODA LA LIBRERÃA...'}
                   value={searchQ}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 20px 140px' }}>
              ...
              {filteredCats.map(cat => (
                <div key={cat} onClick={() => navTo({ view: 'albums', cat })}
                  ...
                  <div>
                    ...
                    <p style={{ fontFamily: "'Space Mono',monospace", fontSize: 9, color: C.orange, margin: '2px 0 0', letterSpacing: '0.1em' }}>{Object.keys(sectionData[cat] || {}).length} ÃLBUMES</p>
                  </div>
          ...
          {tab === 'playlists' && view === 'categories' && !loading && (
            <div>
              ...
              {playlists.length === 0 && <div style={{ padding: '40px 0', textAlign: 'center', fontFamily: "'Space Mono',monospace", fontSize: 10, color: C.brown }}>NO HAY PLAYLISTS AÃšN</div>}
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, height: 'calc(100vh - 100px)', width: 280, background: C.dark, borderLeft: `3px solid ${C.orange}`, zIndex: 40, display: 'flex', flexDirection: 'column', transform: showQueue ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 0.3s cubic-bezier(0.4,0,0.2,1)' }}>
        <div style={{ padding: '16px', borderBottom: `2px solid ${C.orange}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 10, fontWeight: 700, color: C.cream, letterSpacing: '0.15em', textTransform: 'uppercase' }}>COLA Â· {queue.length}</span>
          </div>
        ...
        {queue.length > 0 && (
          <div style={{ padding: '8px 14px', borderBottom: `1px solid ${C.brown}` }}>
            <p style={{ fontFamily: "'Space Mono',monospace", fontSize: 7, color: C.gold, letterSpacing: '0.2em', textTransform: 'uppercase', margin: 0 }}>PRÃ“XIMAS</p>
          </div>
        )}
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {queue.length === 0 && <p style={{ padding: 20, fontFamily: "'Space Mono',monospace", fontSize: 9, color: C.brown, textAlign: 'center', letterSpacing: '0.1em' }}>LA COLA ESTÃ VACÃA</p>}
        </div>
        {tracksRef.current.length > 0 && current && (
          <div style={{ borderTop: `2px solid ${C.brown}` }}>
            <div style={{ padding: '8px 14px', borderBottom: `1px solid ${C.brown}` }}>
              <p style={{ fontFamily: "'Space Mono',monospace", fontSize: 7, color: C.muted, letterSpacing: '0.2em', textTransform: 'uppercase', margin: 0 }}>SIGUIENTE DEL ÃLBUM</p>
            </div>
            {(() => {
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: ctx.y, left: ctx.x, zIndex: 100, background: C.dark, border: `2px solid ${C.orange}`, boxShadow: `4px 4px 0 ${C.dark}`, minWidth: 190 }} onClick={e => e.stopPropagation()}>
            ...
            {[
              ...
              { label: 'Reproducir despuÃ©s', Icon: Zap, action: () => playNext(ctx.track) },
              { label: 'AÃ±adir a la cola', Icon: Plus, action: () => addToQueue(ctx.track) },
            ...
            {playlists.length > 0 && (
              <>
                <div style={{ padding: '6px 12px', borderBottom: `1px solid ${C.brown}` }}>
                  <p style={{ fontFamily: "'Space Mono',monospace", fontSize: 7, color: C.brown, letterSpacing: '0.15em', textTransform: 'uppercase', margin: 0 }}>AÃ‘ADIR A PLAYLIST</p>
                </div>
                {playlists.map(pl => (
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, width: '100%', background: C.dark, borderTop: `3px solid ${C.orange}`, boxShadow: `0 -3px 0 ${C.brown}`, zIndex: 50, height: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '10px 16px', gap: 12 }}>
          <div style={{ flex: '0 0 auto', width: '25%', minWidth: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ fontFamily: "'Chakra Petch',sans-serif", fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: current ? C.cream : C.brown, margin: 0, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{playbackErr ? 'âš  ' : ''}{current?.title || 'STANDBY'}</p>
              <p style={{ fontFamily: "'Space Mono',monospace", fontSize: 8, color: C.gold, margin: 0, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{current ? ((current.artist && current.artist !== 'Desconocido') ? current.artist : current.album) : 'â€”'}</p>
            </div>
